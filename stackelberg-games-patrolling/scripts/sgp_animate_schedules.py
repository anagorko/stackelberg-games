...
class ScheduleAnimation(manim.MovingCameraScene):
    def __init__(self, rollout: sgp.Schedule, map_tile_service: str = None, speed: int = None):
        super().__init__()
        ...
        try:
            self.mts = sgp.MapTileService(map_tile_service)
        except ValueError as e:
            print(f'{e}. {colored("Falling back to OSM tile provider.", "red")}.')
            self.mts = sgp.MapTileService('osm')
    ...
    def construct(self):
        ...
        for node in self.rollout.data.targets:
            ...
            targets[node].move_to(self.transform(self.positions[node]))
        ...
        coverage_interpolation = {target: sgp.PWLMap(coverage[target]) for target in targets}
...
def main():
    ...
    parser.add_argument('plan', default=defaults['defense_plan'], nargs='?',
